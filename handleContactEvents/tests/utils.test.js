...
describe("Test isDisconnectedEventForAbandonedCall", () => {
  ...
  it("should be disconnected event", () => {
    expect(utils.isDisconnectedEventForAbandonedCall(event)).toStrictEqual(
      true
    );
  });
...
describe("Test isRoutingCriteriaExpiredEventForCall", () => {
  ...
  it("should be routing expired event", () => {
    expect(utils.isRoutingCriteriaExpiredEventForCall(event)).toStrictEqual(
      true
    );
  });
  ...
  it("should not be routing expired event : matchCriteria is null", () => {
    event.detail.routingCriteria.steps[0].expression.attributeCondition.matchCriteria =
      null;
    expect(utils.isRoutingCriteriaExpiredEventForCall(event)).toStrictEqual(
      false
    );
  ...
  it("should not be routing expired event : updatedProperties does not include RoutingCriteria.Step.Status", () => {
    event.detail.updatedProperties = ["SomeOtherProperty"];
    expect(utils.isRoutingCriteriaExpiredEventForCall(event)).toStrictEqual(
      false
    );
  });

  it("should not be routing expired event : updatedProperties is null", () => {
    event.detail.updatedProperties = null;
    expect(utils.isRoutingCriteriaExpiredEventForCall(event)).toStrictEqual(
      false
    );
  });

  it("should not be routing expired event : updatedProperties is empty", () => {
    event.detail.updatedProperties = [];
    expect(utils.isRoutingCriteriaExpiredEventForCall(event)).toStrictEqual(
      false
    );
  });
